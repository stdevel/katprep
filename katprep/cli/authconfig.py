    ...
    for hostname in CONTAINER.get_hostnames():
        ...
        print(
            "{} (Username: {} / Password: {})".format(
                hostname, credentials.username, password
            )
        )
...
def remove(options):
    ...
    while options.entry_hostname == "":
        # prompt for hostname
        options.entry_hostname = input("Hostname: ")
    LOGGER.debug("Removing entry hostname='%s'...", options.entry_hostname)
...
def parse_options(args=None):
    ...
    gen_opts.add_argument(
        ...
        metavar="FILE",
        nargs=1,
        help="An authentication container",
        type=str,
    )
